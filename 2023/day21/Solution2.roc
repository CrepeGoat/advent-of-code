...
transposeMap : ValidatedMap -> ValidatedMap
...
flipMapAlongY : ValidatedMap -> ValidatedMap
...
bfsStep : ValidatedMap, List MapIndex -> Set MapIndex
bfsStep = \vmap, poss ->
...
bfsFromPos : ValidatedMap, MapIndex -> List MapIndex
bfsFromPos = \@ValidatedMap { map, bounds, start: _ }, pos ->
    wrappedPos <- getWrappedPosAdjs bounds pos |> List.dropIf
...
getWrappedPosAdjs : MapIndex, MapIndex -> List MapIndex
getWrappedPosAdjs = \bounds, pos ->
    [North, South, East, West] |> List.map (\dir -> wrappedPosToThe bounds pos dir)

wrappedPosToThe : MapIndex, MapIndex, Cardinal -> MapIndex
wrappedPosToThe = \{ x: boundX, y: boundY }, { x, y }, dir ->
    when dir is
        North -> { y: (y + (boundY - 1)) % boundY, x }
        South -> { y: (y + 1) % boundY, x }
        West -> { y, x: (x + (boundX - 1)) % boundX }
        East -> { y, x: (x + 1) % boundX }
